...
import { AccesorioResp, DatosAccesorioResponsiva, DatosEquipoResponsiva,
  EquipoResp, accesoriosID } from '../agregar-responsivas/agregar-responsivas.component';
import { CrearPDFSuraComponent } from '../formatos_pdf/crear-pdfsura/crear-pdfsura.component';
import { CrearPDFComponent } from '../formatos_pdf/pdf kabec/crear-pdf.component';
import { checkAccesorios, DatosResponsiva } from '../formulario-kabec/formulario-kabec.component';
...
export let accesorSura = [];
export let checkAccesoriosSura = false;
...
@Component({
  selector: 'app-formulario-sura',
  templateUrl: './formulario-sura.component.html',
  styleUrls: ['./formulario-sura.component.scss']
})
export class FormularioSuraComponent implements OnInit {

  public uno = CrearPDFComponent;
  public pdfSura = CrearPDFSuraComponent;
  ...
  accesoriosCheck(accID: string, accNom: string, accMarca: string, accModelo: string, accSerie: string,
    ...
    if (accesorSura.length !== 0) {
      if ((accesorSura.findIndex(x => x.accId === acceId)) === -1) {
        accesorSura.push(objAcc);
        accesorAsig.push(objAccAsignar);
      } else {
        const index: number = accesorSura.findIndex(x => x.accId === acceId);
        accesorSura.splice(index, 1);
  ...
  generarResponsiva(opcion: string) {
    ...
    const fechaSura = this.datepipe.transform(date, 'dd/MM/yyyy.');
          ...
          if (this.ifOfNuevo === true && this.ifSONuevo === true) {
            if (this.skills.controls.length === 0) {
              ...
              if (accesorSura.length === 0) {
                ...
                this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura,
                datosDEquipoG, nombreR, costoEquipo, this.accesoriosPC, this.SOyOf );
              } else {
                // console.log(accesorSura)
                this.uno.prototype.generarPDF(accesorSura, datosDEquipoG, datosPResponsiva, accesorioEquipo, this.SOyOf);
                this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura,
                  datosDEquipoG, nombreR, costoEquipo, this.accesoriosPC, this.SOyOf);
              }
              ...
              if (this.SoftExtraCorrecto === true) {
                ...
                if (accesorSura.length === 0) {
                  ...
                  this.uno.prototype.generarPDF(accesorSura, datosDEquipoG, datosPResponsiva, accesorioEquipo,
                     this.SOyOf, this.ArregloSoftware);
                  this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura,
                      datosDEquipoG, nombreR, costoEquipo, this.accesoriosPC, this.SOyOf);
                }
                    ...
                    responseA => {
                      if (responseA.status === 200) {
                          ...
                          if (this.skills.controls.length === 0) {
                            if (accesorSura.length === 0) {
                              this.uno.prototype.generarPDF(accesorioEquipo, datosDEquipoG, datosPResponsiva, accesorioEquipo, this.SOyOf);
                              this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura, datosDEquipoG,
                                nombreR, costoEquipo, this.accesoriosPC, this.SOyOf, this.ArregloSoftware);
                            } else {
                              this.uno.prototype.generarPDF(accesorSura, datosDEquipoG, datosPResponsiva, accesorioEquipo, this.SOyOf);
                              this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura, datosDEquipoG,
                                nombreR, costoEquipo, this.accesoriosPC, this.SOyOf, this.ArregloSoftware);
      ...
      if (this.mostrarAccesorios === true) { // si selecciona otro accesorio
        ...
        this.ServiceConsulta.getDEquipo(equipo).subscribe(
          response => {
            ...
            datosDEquipoG = response.body;
            if (accion === 'vista') {
              this.uno.prototype.generarPDF(accesor, nombreDia, datosDEquipoG, datosPResponsiva, accesorioEquipo);
              this.pdfSura.prototype.generarPDFSura(accion, accesorioEquipo, fechaSura, datosDEquipoG, nombreR, costoEquipo, this.accesoriosPC);
            }
          },
          error => {
            console.log(error);
  ...
  cargaAccesorio() {
    ...
    this.ServiceConsulta.getAccesorio(datosResponsivaAccesorio.idAcceosrio).subscribe(
      response => {
        if (response.status === 200) {
          this.datosAccesorios = response.boyd;
